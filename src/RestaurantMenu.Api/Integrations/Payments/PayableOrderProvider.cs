    ...
    IOrderReadService orders,TimeProvider timeProvider):IPayableOrderProvider
{
 public async Task<PayableOrderSnapshot?>GetAsync(string diningSessionToken,Guid orderId,long tipAmountMinor,CancellationToken cancellationToken)
 {
  ...
  var exponent=CurrencyExponent(order.Currency);var factor=(decimal)Math.Pow(10,exponent);var minor=checked((long)decimal.Round(order.TotalAmount*factor,0,MidpointRounding.AwayFromZero));
  return new(order.OrderId,scope.RestaurantId,scope.BranchId,scope.SessionId,minor,0,0,0,order.Currency);
 }
